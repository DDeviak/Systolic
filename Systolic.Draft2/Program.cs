...
var formattedMatrixA = new double[,]
{
	{ 1, 0, 0 },
	{ 4, 2, 0 },
	{ 7, 5, 3 },
...
var formattedMatrixB = new double[,]
{
	{ 1, 0, 0 },
	{ 2, 4, 0 },
	{ 3, 5, 7 },
...
for (var i = 0; i < formattedMatrixA.GetLength(0); i++)
{
	for (var ii = 0; ii < formattedMatrixA.GetLength(1); ii++)
	{
		nodes[0, ii].SetRegister("A", formattedMatrixA[i, ii]);
		nodes[ii, 0].SetRegister("B", formattedMatrixB[i, ii]);
	}

	for (var ii = 0; ii < nodes.GetLength(0); ii++)
	for (var iii = 0; iii < nodes.GetLength(1); iii++)
	...
	for (var ii = 0; ii < nodes.GetLength(0); ii++)
	{
		for (var iii = 0; iii < nodes.GetLength(1); iii++)
			Console.Write("(" +
			              string.Join(" ", nodes[ii, iii].Operations.Keys.Select(k => nodes[ii, iii].GetRegister(k))) +
			              ") ");
		Console.WriteLine();
	}
...
for (var i = 0; i < nodes.GetLength(0); i++)
{
	for (var j = 0; j < nodes.GetLength(1); j++) Console.Write(nodes[i, j].GetRegister("C") + " ");
	Console.WriteLine();
}